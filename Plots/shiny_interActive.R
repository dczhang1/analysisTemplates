...
server<-function(input, output, session) {
        ...
        observeEvent(input$go, { #Once the "go" button is hit, InterActive looks at all the ui input and runs the model.
                ...
                output$modplot <- renderPlot({
                        ...
                        covarstemp<-input$covars
                        covars<-covarstemp[!covarstemp %in% c(foc,mod)]
                        
                        cov<-paste(covars, collapse=" + ")
                        
                        values$covars<-covars
                        ...
                        if(length(covars)==1){
                                covmeans<-mean(data[,covars],na.rm=TRUE)
                                matxi1[,covars]<-rep(covmeans,length(focal.seq))
                                values$covmeans<-covmeans
                        }
                        else if (length(covars)>1){
                                covmeans<-lapply(data[,covars],mean, na.rm=TRUE)
                                for (i in 1:length(covmeans)){matxi1[,covars[i]]<-rep(covmeans[i],ncol(matxi1))}
                                values$covmeans<-covmeans
                        }
